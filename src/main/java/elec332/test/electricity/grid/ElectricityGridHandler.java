...
import elec332.test.electricity.simulation.engine.Circuit;
import elec332.test.electricity.simulation.engine.SimulationEngine;
...
public final class ElectricityGridHandler extends AbstractGridHandler<ETileEntityLink> implements IElectricityGridHandler {
    ...
    protected void internalAdd(ETileEntityLink o) {
        ...
        Collection<CircuitElement<?>> elm = CircuitElementFactory.INSTANCE.wrapComponent(tile.getCapability(TestModAPI.ELECTRICITY_CAP));
        elm.forEach(element -> {
            map.put(o, element);
            addElmImpl(element);
        });
    ...
    public boolean isValidObject(TileEntity tile) {
        LazyOptional<IElectricityDevice> cap = tile.getCapability(TestModAPI.ELECTRICITY_CAP);
        if (!cap.isPresent()) {
            return false;
        }
