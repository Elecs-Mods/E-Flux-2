...
public enum SimulationEngine {
    ...
    public void tick(Circuit circuit) {
        ...
        synchronized (circuit) {
            ...
            double dt = (System.nanoTime() - time) / 10e6;
            if (debug()) {
            ...
            runCircuit(circuit);
            if (debug()) {
                double d = (System.nanoTime() - time) / 10e6;
                System.out.println("Circuit simulation: " + circuit.getCompressedElementList().size());
                System.out.println(circuit.getCompressedElementList());
                System.out.println("Circuit tick " + "took: " + d + "ms");
    ...
    private void analyzeCircuit_(Circuit circuit) {
        synchronized (circuit) {
            ...
            elmList.forEach(CircuitElement::preAnalyze);
    ...
    void runCircuit(Circuit circuit) {
        ...
        for (int c = 0; c < 1; c++) {
            ...
            for (subiter = 0; subiter != subiterCount; subiter++) {
                ...
                if (printit) {
                    ...
                    System.out.println();
                    System.out.println(circuit.circuitMatrixSize + "  " + circuit.circuitNonLinear);
                }
                if (circuit.circuitNonLinear) {
        ...
        for (ICircuitPart cp : circuit.getCircuitParts()) {
            for (ISubCircuitChecker checker : CircuitElementFactory.INSTANCE.getCircuitCheckers()) {
                if (!checker.isSubCircuitValid(cp)) {
                    return;
                }
            }
        }
    ...
    boolean lu_factor(Circuit circuit) {
        ...
        for (j = 0; j != n; j++) {
            ...
            // calculate upper triangular simulation for this column
            for (i = 0; i != j; i++) {
                double q = a[i][j];
                for (k = 0; k != i; k++) {
                    q -= a[i][k] * a[k][j];
            ...
            // calculate lower triangular simulation for this column
            double largest = 0;
            int largestRow = -1;
            for (i = j; i != n; i++) {
                double q = a[i][j];
