 ...
 */
@OnlyIn(Dist.CLIENT)
public enum OverHeadWireHandlerClient implements INetworkObjectReceiver {

    INSTANCE;

    private OverHeadWireHandlerClient() {
    ...
    public void onPacket(int id, ElecByteBuf data) {
        switch (id) {
            case 0:
                loadedWires.clear();
                NBTTagList list = Preconditions.checkNotNull(data.readCompoundTag()).getList("wires", NBTTypes.COMPOUND.getID());
