...
public class TerminalRenderer {
    ...
    private TerminalRenderer() {
        ...
        for (int i = 0; i < count; i++) {
            mrl[i] = new ModelResourceLocation("eleccore" + ":wireterm" + (i + 1), "normal");
    ...
    public void afterAllModelsBaked(ModelLoadEvent event) throws Exception {
        for (int i = 0; i < mrl.length; i++) {
            ...
            models[i] = model.bake(ModelLoader.defaultModelGetter(), ModelLoader.defaultTextureGetter(), model.getDefaultState(), false, DefaultVertexFormats.BLOCK);
        }
    }
    ...
    public static void getTerminalQuads(List<BakedQuad> ret, EnumFacing side, Iterable<GroundTerminal> terminals, boolean item) {
        ...
        for (GroundTerminal gt : terminals) {
            ...
            for (BakedQuad quad : model.getQuads(null, side, new Random())) {
                ...
                if (clr != -1) {
                    EnumDyeColor cl = gt.getColor();
                    if (cl != null) {
                        clr = cl.func_196060_f();
                    } else {
                        clr = -1;
                    }
                }
