...
public class ItemGroundWire extends ItemSubTile implements INoJsonItem {
    ...
    public void afterBlockPlaced(@Nonnull BlockItemUseContext context, @Nonnull BlockState state, TileEntity tile) {
        if (!context.getWorld().isRemote) {
            ...
            IWireContainer wireHandler = tile.getCapability(WiresModule.WIRE_CAPABILITY).orElse(null);
    ...
    public void onEmptySolidSideClicked(@Nonnull World world, @Nonnull BlockPos clickedPos, @Nonnull TileEntity tile, @Nonnull Direction hit, PlayerEntity player, ItemStack stack, BlockState state) {
        IWireContainer wireHandler = tile.getCapability(WiresModule.WIRE_CAPABILITY, null).orElse(null);
    ...
    public static ItemStack withCables(@Nonnull Collection<DyeColor> colors, int size) {
        ...
        ItemStack ret = new ItemStack(WiresModule.GROUND_WIRE_ITEM.get(), 1);
        ret.setTag(tag);
        return ret;
    }

    public static Pair<Integer, Integer> getColorsDataFromStack(@Nonnull ItemStack stack) {
        if (stack.getItem() != WiresModule.GROUND_WIRE_ITEM.get()) {
