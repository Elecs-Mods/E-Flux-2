...
public class WireRenderer implements IModelAndTextureLoader {
    ...
    public void registerTextures(IIconRegistrar iiconRegistrar) {
        ...
        for (int i = 0; i < wireTypes.length; i++) {
            wireTypes[i] = iiconRegistrar.registerSprite(new EFlux2ResourceLocation("block/flatwire_" + EnumElectricityType.values()[i].toString().toLowerCase()));
        }

        black = iiconRegistrar.registerSprite(new EFlux2ResourceLocation("block/black"));
    }
