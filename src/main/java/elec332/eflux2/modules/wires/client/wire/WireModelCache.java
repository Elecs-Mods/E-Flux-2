...
import elec332.eflux2.modules.wires.client.ModelProperties;
...
public class WireModelCache extends ModelCache<WireModelCache.WireRenderData> {
    ...
    public void addModelData(@Nonnull ILightReader world, @Nonnull BlockPos pos, @Nonnull BlockState state, @Nonnull IModelData modelData) {
        ...
        modelData.setData(ModelProperties.WIRE, tile.getCapability(WiresModule.WIRE_CAPABILITY).orElseThrow(NullPointerException::new).getWireView());
        modelData.setData(ModelProperties.TERMINAL, tile.getCapability(WiresModule.TERMINAL_CAPABILITY).orElseThrow(NullPointerException::new).getTerminalView());
    }

    @Override
    protected WireRenderData get(ItemStack stack) {
        Item item = stack.getItem();
        if (item == WiresModule.GROUND_WIRE_ITEM.get()) {
    ...
    protected void bakeQuads(List<BakedQuad> quads, Direction side, WireRenderData key) {
        ...
        if (!key.terminals.isEmpty()) {
            TerminalRenderer.makeQuads(quads, side, key.terminals);
        }
    }
