...
public class SubTileTerminal extends SubTileLogicBase implements ISubTileTerminal, IElectricityDevice, IEnergyObject, IWireContainer {
    ...
    private final BitSet bcfs = new BitSet(Direction.values().length);

    @Override
    public boolean addTerminal(GroundTerminal terminal) {
        BlockPos pos = new IndexedBlockPos(getPos(), 1);
        VoxelShape block = WorldHelper.getShape(getWorld(), pos);
        if (!block.isEmpty() && HitboxHelper.collides(block, terminal.getShape())) {
            return false;
        }
...
//    }
    ...
    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction facing) {
        if (cap == WiresModule.WIRE_CAPABILITY) {
            return LazyOptional.of(() -> this).cast();
        }
        return cap == EFlux2API.ELECTRICITY_CAP ? connections.cast() : WiresModule.TERMINAL_CAPABILITY.orEmpty(cap, LazyOptional.of(() -> this));
    }
    ...
    public boolean addWire(GroundWire wire) {
        if (!wire.isTerminalPart() && bcfs.get(wire.getPlacement().ordinal())) {
            wire.setIsTerminal();
            return WorldHelper.getTileAt(getWorld(), getPos()).getCapability(WiresModule.WIRE_CAPABILITY)
