...
public class SubTileWire extends SubTileLogicBase implements ISubTileLogic, IWireContainer, IElectricityDevice {
    ...
    private boolean addWireInternal(@Nonnull GroundWire wire, boolean notify) {
        ...
        if (wireView.isEmpty() || getWire(wire.getPlacement()) == null) {
            if (!wire.isTerminalPart() && notify && HitboxHelper.collides(WorldHelper.getShape(getWorld(), getPos()), wire.getShape())) {
                return false;
            }
            wires.add(wire);
            wire.wire = this;
            added = true;
            resetWireCapability();
            if (hasWorld() && WorldHelper.isServer(getWorld())) {
                ...
                for (int k = 0; k < 3; k++) {
                    if (i == 1 || j == 1 || k == 1) {
                        ...
                        if (!getPos().equals(poz)) {
                            getWorld().neighborChanged(poz, WiresModule.WIRE_MARKER, getPos());
                        }
                    }
                }
            }
    ...
    public boolean removedByPlayer(@Nonnull PlayerEntity player, boolean willHarvest, @Nonnull RayTraceResult hit) {
        if (WorldHelper.isServer(getWorld()) && hit.hitInfo instanceof GroundWire) {
            GroundWire wirePart = (GroundWire) hit.hitInfo;
            if (wirePart != null) {
                remove(wirePart);
                ItemStack stack = wirePart.getDropStack();
                WorldHelper.dropStack(getWorld(), getPos(), stack);
                return true;
            }
        }
        return false;
    ...
    @Override
    public ActionResultType onBlockActivated(PlayerEntity player, Hand hand, RayTraceResult hit) {
    ...
    public void neighborChanged(BlockPos neighborPos, Block changedBlock, boolean observer) {
        ...
        for (GroundWire p : getWireView()) {
            if (!p.canStay(getWorld(), getPos())) {
                WorldHelper.dropStack(getWorld(), getPos(), p.getDropStack());
                wp.add(p);
            }
        }
        removeAll(wp);
        if (wp.isEmpty()) {
            checkConnections(false, changedBlock != WiresModule.WIRE_MARKER);
        }
    }
    ...
    private void checkConnections(boolean forcePacket, boolean forceCheck) {
        ...
        Function<BlockPos, Pair<Boolean, IWireContainer>> func = new Function<BlockPos, Pair<Boolean, IWireContainer>>() {
            ...
            private final Map<BlockPos, Boolean> prevData = Maps.newHashMap();
    ...
    public void invalidate() {
        if (hasWorld() && WorldHelper.chunkLoaded(getWorld(), getPos())) {
            ...
            if (state.getBlock() != WiresModule.GROUND_WIRE_BLOCK.get() && myWire != null) {
                myWire.invalidate();
                notifyNeighborsOfChangeExtensively();
            }
        }
    ...
    private Pair<Boolean, IWireContainer> getWire(BlockPos pos) {
        ...
        if (w == null || !w.isPresent()) {
            ...
            if (tile != null) {
                w = tile.getCapability(WiresModule.WIRE_CAPABILITY);
                if (w.isPresent()) {
    ...
    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
        ...
        return WiresModule.WIRE_CAPABILITY.orEmpty(cap, myWire);
    }
